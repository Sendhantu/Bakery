...
def enrich_products(products):
    ...
    review_stats = {
        product_id: {"count": int(count or 0), "avg": round(float(avg or 0), 1)}
    ...
    for product in products:
        ...
        variant_info = variant_stats.get(
            product.id, {"default_variant_id": None, "total_stock": 0}
        )
...
def enrich_orders(orders):
    ...
    item_counts = {
        order_id: int(item_count or 0) for order_id, item_count in item_count_rows
    }
...
def get_customer_products_page(filters, page, per_page):
    ...
    if q:
        ...
        try:
            ...
            if cat_id:
                filter_arr.append(f"category_id = {cat_id}")
            if min_price:
                filter_arr.append(f"price >= {min_price}")
            if max_price:
                filter_arr.append(f"price <= {max_price}")
            ...
            if product_ids:
                query = Product.query.options(selectinload(Product.category)).filter(
                    Product.id.in_(product_ids)
                )
                items = query.all()
                items.sort(key=lambda item: product_ids.index(item.id))
...
def get_admin_products_page(search, page, per_page):
    ...
    pagination = (
        query.order_by(Product.is_active.desc(), Product.created_at.desc())
        .paginate(page=page, per_page=per_page, error_out=False)
    )
    enrich_products(pagination.items)
    return pagination
...
def get_admin_raw_materials_page(search, page, per_page):
    ...
    return query.order_by(RawMaterial.is_active.desc(), RawMaterial.name.asc()).paginate(
        page=page, per_page=per_page, error_out=False
    )
...
def get_admin_coupons_page(search, page, per_page):
    ...
    pagination = query.order_by(Coupon.id.desc()).paginate(
        page=page, per_page=per_page, error_out=False
    )
...
def get_admin_agents():
    ...
    if agent_ids:
        delivery_rows = (
            db.session.query(Delivery.agent_id, func.count(Delivery.id))
            .filter(Delivery.agent_id.in_(agent_ids))
            .group_by(Delivery.agent_id)
            .all()
        )
        delivery_counts = {
            agent_id: int(count or 0) for agent_id, count in delivery_rows
        }
...
def get_category_summaries():
    ...
    if category_ids:
        ...
        product_counts = {
            category_id: int(count or 0) for category_id, count in rows
        }
