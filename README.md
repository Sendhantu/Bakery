...
The `render.yaml` deploys only the customer web service plus its Celery worker. Admin and delivery portals remain local and must point at the same TiDB database and the same Redis/Key Value instance for live Socket.IO sync.

The web service and worker receive Render Key Value internal connection strings automatically. For your local admin and delivery portals, enable external connections on the same Render Key Value instance and copy its external TLS URL into your local `.env`, or use one shared internet-reachable Redis provider such as Upstash or Redis Cloud for both Render and local portals.

You need to manually set these external secrets in the Render dashboard:

**TiDB Database (Required):**
- `DATABASE_URL`: `mysql+pymysql://user:password@gateway01.ap-southeast-1.prod.aws.tidbcloud.com:4000/bakerydb`
- SSL settings are already included in `render.yaml` for TiDB Cloud.

**Finance encryption (Required if finance records exist):**
- `FINANCIAL_DATA_ENCRYPTION_KEY`: generate once with `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`
- Use the same value in Render and in the local admin/delivery `.env` files. Do not rotate it unless you have a re-encryption plan.
...
- The worker service inherits the same TiDB, Redis, finance encryption, Cloudinary, mail, Twilio, Firebase, Google Maps, and Sentry settings as the customer portal.

#### 4A. Configure Local Admin and Delivery for Hybrid Live Sync

On the local machine that runs admin/delivery, keep `PORTAL_ROLE` local to each process but share the same cloud data services:
...
DB_SSL_VERIFY_CERT=true
DB_SSL_VERIFY_IDENTITY=true
FINANCIAL_DATA_ENCRYPTION_KEY=<same-value-as-render>

# Use the external TLS URL for the same Render Key Value instance,
# or use the same Upstash/Redis Cloud URL configured on Render.
REDIS_URL=rediss://default:password@external-redis-host:port
SOCKETIO_MESSAGE_QUEUE=rediss://default:password@external-redis-host:port
CELERY_BROKER_URL=rediss://default:password@external-redis-host:port
CELERY_RESULT_BACKEND=rediss://default:password@external-redis-host:port
RATELIMIT_STORAGE_URI=rediss://default:password@external-redis-host:port

OFFLINE_SYNC_ENABLED=true
ENABLE_LOCAL_SYNC_WORKER=true
CUSTOMER_PORTAL_URL=https://bakery-customer-portal.onrender.com
ADMIN_PORTAL_URL=http://127.0.0.1:5001
DELIVERY_PORTAL_URL=http://127.0.0.1:5002
```

Then start local portals with:

```bash
python run_dev_portals.py
```

This stores confirmed online data in TiDB. When the admin or delivery portal is offline, updates are queued in the local SQLite offline queue under `instance/offline/` and replayed to TiDB when connectivity returns. The local SQLite queue is a recovery buffer, not a second production database.
...
| **Redis connection error** | Verify Render Key Value is linked internally and local portals use an allowlisted external Redis URL |
...
The `render.yaml` automatically provisions a Render Key Value instance:
...
The internal Redis-compatible URL is automatically linked to the Render web and worker services. For local admin/delivery live sync, enable external connections for the same instance, allowlist the local network IP address, and use the external `rediss://...` URL locally.
...
# Render services (auto-linked from render.yaml)
REDIS_URL=redis://default:password@host:port

# Local admin/delivery connecting to Render Key Value external URL
REDIS_URL=rediss://default:password@external-host:port
