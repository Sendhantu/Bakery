...
def build_database_uri(allow_sqlite_fallback=True):
    direct_url = normalize_database_uri(os.environ.get("DATABASE_URL"))
    if direct_url:
        return direct_url

    db_host = (os.environ.get("DB_HOST") or "").strip()
    db_port = (os.environ.get("DB_PORT") or "4000").strip()
    ...
    mysql_database_url = normalize_database_uri(os.environ.get("MYSQL_DATABASE_URL"))
