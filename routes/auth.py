...
from utils import save_address_for_customer, get_saved_addresses_for_user, send_email
import os
...
def password_reset_serializer():
    return URLSafeTimedSerializer(current_app.config['SECRET_KEY'], salt='sweetcrumbs-password-reset')
...
def record_login(user, status='success'):
    ...
    db.session.add(LoginHistory(
        user_id=user.id, ip_address=ip, device=ua, status=status
    ))
...
def get_recent_failed_attempts(user):
    ...
    last_success = LoginHistory.query.filter_by(user_id=user.id, status='success').filter(
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and password_ok and user.is_active:
            ...
            merged_items = merge_guest_cart_into_user(user.id) if user.role == 'customer' else 0
            record_login(user, 'success')
            db.session.commit()
            if current_portal_role() != expected_portal:
                flash(f'Signed in successfully. Redirecting you to the {expected_portal} portal.', 'info')
            ...
            if user:
                failure_status = 'inactive' if password_ok and not user.is_active else 'failed'
                record_login(user, failure_status)
                db.session.commit()
                if failure_status == 'inactive':
                    current_app.logger.warning('Inactive account login attempt for %s', user.email)
...
def authorize_google():
    ...
    if not user:
        ...
        if user:
            ...
            user = User(email=email, name=name, oauth_id=google_id, oauth_provider='google', role='customer')
            db.session.add(user)
...
def register():
    ...
    if current_portal_role() != 'customer':
        flash('Customer registration is available only on the storefront portal.', 'info')
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            db.session.add(user)
            db.session.flush()
            record_development_credential('customer', email, password, label=f'Customer Account ({name})', source='registered')
            login_user(user)
            from routes.customer import merge_guest_cart_into_user
...
def forgot_password():
    ...
    if current_portal_role() != 'customer':
        flash('Customer passwords can be reset from the storefront. Admin and delivery credentials are managed in the operations portal.', 'info')
        return redirect(url_for('auth.login'))
    ...
    if request.method == 'POST':
        ...
        if user and user.is_active:
            token = password_reset_serializer().dumps({'user_id': user.id, 'email': user.email})
            reset_link = build_password_reset_link(token)
            reset_minutes = int(current_app.config.get('PASSWORD_RESET_TOKEN_MINUTES', 30))
            html = f"""
            ...
            """
            send_email(user.email, 'Reset your SweetCrumbs password', html, text_body=reset_link)
            if (current_app.debug or current_app.testing) and not current_app.config.get('MAIL_ENABLED'):
...
def reset_password(token):
    ...
    if current_portal_role() != 'customer':
        return redirect(portal_url_for_role('customer', url_for('auth.reset_password', token=token)))
    ...
    if request.method == 'POST':
        ...
        user.set_password(password)
        from app import record_development_credential
        record_development_credential('customer', user.email, password, label=f'Customer Account ({user.name})', source='password_reset')
        db.session.commit()
...
def profile():
    if request.method == 'POST':
        ...
        if new_pw:
            ...
            from app import record_development_credential
            record_development_credential(current_user.role, current_user.email, new_pw, label=f'{current_user.role.title()} Account ({current_user.name})', source='profile_update')
        db.session.commit()
    ...
    return render_template('auth/profile.html', history=history, saved_addresses=saved_addresses)
...
def add_saved_address():
    ...
    if not all(payload[field] for field in ('address_line1', 'city', 'pincode', 'phone')):
...
def set_default_address(address_id):
    address = SavedAddress.query.filter_by(id=address_id, user_id=current_user.id).first_or_404()
...
def delete_saved_address(address_id):
    ...
    if was_default:
        next_address = SavedAddress.query.filter_by(user_id=current_user.id).order_by(
