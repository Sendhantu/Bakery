...
from models import (db, User, Product, ProductVariant, Category, Order, OrderItem,
                    Payment, Refund, Coupon, Subscription, Message, Notification,
                    DeliveryAgent, Delivery, LoginHistory, Review, ModificationRequest,
                    RawMaterial, ProductMaterial, PaymentLink, LoyaltyLedger)
from utils import (parse_decimal, notify, send_status_update_email,
...
def apply_product_image(product):
    ...
    allowed = current_app.config.get('ALLOWED_IMAGE_EXTENSIONS', {'jpg', 'jpeg', 'png', 'webp', 'gif'})
...
def dashboard():
    ...
    today_revenue   = db.session.query(func.sum(Order.total)).filter(
        func.date(Order.placed_at) == today, Order.status != 'CANCELLED').scalar() or 0
    total_customers  = User.query.filter_by(role='customer').count()
    pending_orders   = Order.query.filter(Order.status.in_(['PLACED', 'PREPARING'])).count()
    low_stock_items  = ProductVariant.query.filter(ProductVariant.stock <= 5, ProductVariant.stock > 0).count()
    ...
    current_week_revenue = db.session.query(func.sum(Order.total)).filter(
        func.date(Order.placed_at) >= window_start,
        func.date(Order.placed_at) <= today,
        Order.status != 'CANCELLED',
    ).scalar() or 0
    prev_week_revenue = db.session.query(func.sum(Order.total)).filter(
        func.date(Order.placed_at) >= prev_window_start,
        func.date(Order.placed_at) <= prev_window_end,
        Order.status != 'CANCELLED',
    ).scalar() or 0
    ...
    for i in range(6, -1, -1):
        ...
        rev = db.session.query(func.sum(Order.total)).filter(
            func.date(Order.placed_at) == d, Order.status != 'CANCELLED'
        ).scalar() or 0
    ...
    pending_msgs = Message.query.filter_by(receiver_id=current_user.id, is_read=False).count()
    ...
    return render_template('admin/dashboard.html',
        ...
        low_stock_items=low_stock_items, out_of_stock=out_of_stock,
        inactive_products=inactive_products,
        low_stock_materials=low_stock_materials,
        out_of_stock_materials=out_of_stock_materials,
        total_loyalty_points=int(total_loyalty_points),
        recent_orders=recent_orders, chart_labels=labels,
        chart_revenues=revenues, chart_order_counts=order_counts,
        top_products=top_products, pending_msgs=pending_msgs,
...
def add_product():
    ...
    return render_template('admin/product_form.html', product=None,
                           categories=categories, raw_materials=raw_materials)
    ...
    if request.method == 'POST':
        ...
        for vid, vn, vp, vs in zip(vid_list, vname_list, vprice_list, vstock_list):
            if vid:
                ...
                if v:
                    v.name = vn; v.price = float(vp); v.stock = int(vs or 0)
            elif vn and vp:
                db.session.add(ProductVariant(
                    product_id=p.id, name=vn, price=float(vp), stock=int(vs or 0)
                ))
...
def delete_product(product_id):
    ...
    if p.is_active:
        ...
        return redirect(url_for('admin.products'))
    if p.total_stock <= 0:
        flash('Add stock to at least one variant before making this product live.', 'warning')
...
def orders():
    ...
    return render_template('admin/orders.html', orders=orders,
                           status_filter=status, scope_filter=scope)
...
def order_detail(order_id):
    ...
    agents       = DeliveryAgent.query.order_by(
        DeliveryAgent.availability.desc(), DeliveryAgent.name).all()
    ...
    return render_template('admin/order_detail.html',
                           order=order, items=items, agents=agents,
                           mod_reqs=mod_reqs, addr_hist=addr_hist,
...
def update_order_status(order_id):
    ...
    # In-app notification
    notify(order.user_id, f'Order Update: {status}',
    ...
    if status == 'DELIVERED' and old_status != 'DELIVERED':
        ...
        if pts:
            notify(order.user_id, '🎉 Loyalty Points Earned!',
                   f'You earned {pts} points for order #{order.order_number}.',
                   'loyalty', url_for('customer.loyalty'))
...
def assign_delivery(order_id):
    ...
    if existing:
        ...
        db.session.add(Delivery(order_id=order_id, agent_id=agent_id,
...
@admin_required
def modifications():
    reqs = ModificationRequest.query.filter_by(status='PENDING')\
...
def resolve_modification(req_id):
    ...
    if action == 'approve':
        ...
        if price_diff > 0:
            ...
            notify(req.order.user_id, 'Extra Payment Required',
...
def chat_thread(customer_id):
    ...
    return render_template('admin/chat.html', customers=customers,
                           messages=messages, active_customer=customer)
...
def admin_send_message(receiver_id):
    ...
    if content:
        ...
        notify(receiver_id, 'New Message from Bakery', content[:100],
               'chat', url_for('customer.chat'))
        db.session.commit()
...
@admin_required
def inventory():
    variants = ProductVariant.query.join(Product)\
                .order_by(Product.is_active.desc(), ProductVariant.stock.asc(), Product.name).all()
    materials = RawMaterial.query.order_by(
        RawMaterial.is_active.desc(), RawMaterial.stock.asc(), RawMaterial.name).all()
    live_products      = Product.query.filter_by(is_active=True).count()
    inactive_products  = Product.query.filter_by(is_active=False).count()
    low_variant_count  = ProductVariant.query.filter(ProductVariant.stock > 0, ProductVariant.stock <= 5).count()
    ...
    return render_template('admin/inventory.html',
        variants=variants, materials=materials,
        live_products=live_products, inactive_products=inactive_products,
        low_variant_count=low_variant_count, out_of_stock_count=out_of_stock_count,
...
def raw_materials():
    ...
    return render_template('admin/raw_materials.html',
        materials=materials,
...
def add_raw_material():
    ...
    db.session.add(RawMaterial(
        ...
        stock=stock, reorder_level=reorder, cost_per_unit=cost,
...
def update_raw_material(material_id):
    ...
    try:
        mat.stock         = parse_decimal(request.form.get('stock'), 'stock')
        mat.reorder_level = parse_decimal(request.form.get('reorder_level'), 'reorder level')
        mat.cost_per_unit = parse_decimal(request.form.get('cost_per_unit'), 'cost per unit')
    except ValueError as e:
        flash(str(e), 'danger')
        return redirect(url_for('admin.raw_materials'))
    mat.name     = name
...
def toggle_raw_material_status(material_id):
    ...
    db.session.commit()
    flash('Raw material ' + ('enabled.' if mat.is_active else 'paused.'),
...
def add_coupon():
    ...
    if Coupon.query.filter_by(code=code).first():
        ...
        return redirect(url_for('admin.coupons'))
    try:
        discount_value  = parse_decimal(request.form.get('discount_value'), 'discount value')
        min_order_value = parse_decimal(request.form.get('min_order_value'), 'min order value')
    except ValueError as e:
        flash(str(e), 'danger')
        return redirect(url_for('admin.coupons'))
    valid_until = None
...
@admin_required
def agents():
    agents = DeliveryAgent.query.outerjoin(User, DeliveryAgent.user_id == User.id).order_by(
        User.is_active.desc(),
    ...
    for agent in agents:
        open_deliveries = agent.deliveries.filter(Delivery.status != 'DELIVERED').count()
        completed_deliveries = agent.deliveries.filter(Delivery.status == 'DELIVERED').count()
...
def add_agent():
    ...
    record_development_credential('delivery', email, password, label=f'Delivery Account ({name})', source='admin_created')
    flash(f'Delivery account created for {name}. Credentials: {email} / {password}', 'success')
    current_app.logger.info('Delivery credentials created: %s / %s', email, password)
    return redirect(url_for('admin.agents'))
...
def reset_agent_password(agent_id):
    ...
    from app import record_development_credential
    record_development_credential('delivery', agent.user.email, password, label=f'Delivery Account ({agent.name})', source='admin_reset')
    flash(f'Password reset for {agent.name}. Credentials: {agent.user.email} / {password}', 'success')
    current_app.logger.info('Delivery credentials reset: %s / %s', agent.user.email, password)
    return redirect(url_for('admin.agents'))
...
def toggle_agent_access(agent_id):
    ...
    if not agent.user.is_active:
        ...
        has_open_delivery = agent.deliveries.filter(Delivery.status != 'DELIVERED').first() is not None
...
def analytics():
    ...
    try:
        for i in range(5, -1, -1):
            ...
            monthly_data.append({'month': month_start.strftime('%b %Y'), 'revenue': float(rev)})
    except Exception:
    ...
    # ── Loyalty stats ──
    total_pts_issued = db.session.query(
        func.coalesce(func.sum(LoyaltyLedger.points), 0)
    ).filter(LoyaltyLedger.points > 0).scalar() or 0
    total_pts_redeemed = abs(db.session.query(
        func.coalesce(func.sum(LoyaltyLedger.points), 0)
    ).filter(LoyaltyLedger.points < 0).scalar() or 0)

    # ── Repeat customer rate ──
    repeat_customers = db.session.query(Order.user_id)\
...
def loyalty():
    ...
    top_users = db.session.query(
        User,
        func.sum(LoyaltyLedger.points).label('total_pts')
    ...
    total_issued   = db.session.query(func.coalesce(func.sum(LoyaltyLedger.points), 0))\
                      .filter(LoyaltyLedger.points > 0).scalar() or 0
    total_redeemed = abs(db.session.query(func.coalesce(func.sum(LoyaltyLedger.points), 0))\
                      .filter(LoyaltyLedger.points < 0).scalar() or 0)
...
def loyalty_adjust():
    ...
    notify(user_id, 'Loyalty Points Updated',
